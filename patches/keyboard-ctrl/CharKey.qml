    ...
    PressArea {
        ...
        onReleased: {
            ...
            if (extendedKeysShown) {
                ...
                if (panel.ctrlActive && keyToSend.length === 1) { var code = keyToSend.toUpperCase().charCodeAt(0); if (code >= 64 && code <= 95) { var ctrlChar = String.fromCharCode(code - 64); event_handler.onKeyReleased(ctrlChar, action); panel.ctrlActive = false; } else { event_handler.onKeyReleased(keyToSend, action); panel.ctrlActive = false; } } else { event_handler.onKeyReleased(keyToSend, action); }
                keySent(keyToSend);
